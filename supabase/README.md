...
# الحدّ اليومي لكل مستخدم (٧ تحليلات/يوم)

حماية من التكلفة: يفرض الوسيط حدّاً يومياً لكل مستخدم. التطبيق يرسل توكن المستخدم،
والوسيط يقرأ معرّفه ويزيد عدّاداً في قاعدة البيانات؛ عند بلوغ ٧ يردّ `429` برسالة عربية.

## التفعيل (٣ خطوات)
1. **قاعدة البيانات:** Supabase ← **SQL Editor** ← الصق محتوى `supabase/sql/usage_cap.sql` ← **Run**
   (ينشئ جدول `usage_daily` ودالة `bump_usage`).
2. **أعد نشر الدالة** `analyze` (الكود المحدّث يفرض الحدّ). `SUPABASE_URL` و
   `SUPABASE_SERVICE_ROLE_KEY` متوفّران تلقائياً — لا أسرار إضافية.
3. **أعد بناء التطبيق** (`git pull` ثم Build) كي يرسل التطبيق توكن المستخدم في ترويسة
   `Authorization`. قبل إعادة البناء لا يُفرَض الحدّ (fail-open).

## السلوك والملاحظات
- الحدّ = `DAILY_LIMIT` في `index.ts` (٧). غيّره وأعد النشر إن أردت.
- يُعاد الضبط عند منتصف ليل **UTC**.
- يُحسب عند المحاولة (قبل استدعاء Gemini) فيمنع الاندفاع المتزامن، **ويُسترجَع تلقائياً
  إذا فشل التحليل** (لا تُحتسب المحاولات الفاشلة). إعادة تشغيل `usage_cap.sql` آمنة وتضيف دالة الاسترجاع.

---

# حذف الحساب (متطلّب App Store 5.1.1(v))

دالة `delete-account` تحذف حساب المُتصل وبياناته من الخادم. التطبيق يستدعيها من
**الإعدادات ← الحساب ← حذف الحساب**.

## النشر (مرة واحدة)
1. `git pull` لجلب `supabase/functions/delete-account/index.ts`.
2. **لوحة التحكم:** Edge Functions ← Create a function ← الاسم `delete-account` ←
   الصق الكود ← Deploy. **أو CLI:** `supabase functions deploy delete-account --no-verify-jwt`.
3. **عطّل Verify JWT** لهذه الدالة (نتحقق من التوكن داخلها).
4. لا أسرار إضافية — `SUPABASE_URL` و`SUPABASE_SERVICE_ROLE_KEY` متوفّران تلقائياً.

كيف تعمل: تتحقق من توكن المستخدم عبر `/auth/v1/user`، ثم تحذف صفوف `usage_daily`
الخاصة به، ثم تحذف حساب المصادقة عبر واجهة المشرف (service role). التطبيق بعدها
يحذف البيانات المحلية ويسجّل الخروج.
- **fail-open:** إن تعذّر العدّ (قبل تشغيل SQL مثلاً) يُسمح بالتحليل بدل تعطيله.
- حدّ ناعم: يعتمد على `sub` من التوكن دون تحقق توقيع — رادع كافٍ للتكلفة، وليس حاجزاً أمنياً
  صارماً. للتشديد لاحقاً: App Attest أو إعادة تفعيل Verify JWT.
