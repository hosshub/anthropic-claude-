...
| 8. CI on push | ✅ wired (v1.0.1) | `.github/workflows/flutter-ci.yml` runs `flutter analyze` + `flutter test` on every push that touches `flutter_app/**`. Flutter pinned to 3.29.0 (the first stable that bumped flutter_localizations to intl 0.20.x; our pubspec requires intl ^0.20.2). |
