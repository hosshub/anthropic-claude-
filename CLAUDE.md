...
| Store | State |
|---|---|
| **App Store** | App record created (bundle `ai.tayyibat.tayyibat`). v1.0.2 build 5 uploaded but rejected twice under Guideline 2.1(a): the reviewer hit Gemini's raw "prepayment credits are depleted" billing message in suggest/plan. Root cause = depleted Gemini API credits (account issue, not code). Fixes: (a) server `analyze` function no longer passes provider error text through (deploy from repo root: `supabase functions deploy analyze --no-verify-jwt`), (b) client v1.0.2+6 sanitizes every error path (`sanitizeServerMessage` + `describeError` in `lib/services/app_messages.dart`) so raw provider/exception text can never render. Pending: refill Gemini billing → redeploy function → build 1.0.2 (6) IPA on Mac → upload via Xcode Organizer → attach to the 1.0.2 version in ASC → resubmit + reply to Apple. |
