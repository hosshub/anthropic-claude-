...
| 8. CI on push | ✅ wired (v1.0.1) | `.github/workflows/flutter-ci.yml` runs `flutter analyze` + `flutter test` on every push that touches `flutter_app/**`. Flutter pinned to 3.27.4 (matches local Mac toolchain; carries intl 0.20.x as flutter_localizations requires). |
